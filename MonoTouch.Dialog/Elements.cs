...
namespace MonoTouch.Dialog
{
	...
	public class FloatElement : Element {
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			if (Caption != null && ShowCaption){
				cell.TextLabel.Text = Caption;
				var stringSize = UIKit.UIStringDrawing.StringSize (Caption, UIFont.FromName (cell.TextLabel.Font.Name, UIFont.LabelFontSize));
				captionSize = new CGSize((float)stringSize.Width, (float)stringSize.Height);
	...
	public class StyledMultilineElement : StyledStringElement, IElementSizing {
		...
		public virtual nfloat GetHeight (UITableView tableView, NSIndexPath indexPath)
		{
			...
			CGSize maxSize = new CGSize ((float)(tableView.Bounds.Width - margin), float.MaxValue);
			...
			nfloat height = UIKit.UIStringDrawing.StringSize (c, captionFont, maxSize, LineBreakMode).Height;
			...
			if (!String.IsNullOrEmpty (v)) {
				...
				if (this.style == UITableViewCellStyle.Subtitle) {
					...
					nfloat vheight = UIKit.UIStringDrawing.StringSize (v, subtitleFont, maxSize, LineBreakMode).Height;
					if (vheight > height)
						height = vheight;
				}
			}
	...
	public class MultilineElement : StringElement, IElementSizing {
		...
		{
			float margin = UIDevice.CurrentDevice.UserInterfaceIdiom == UIUserInterfaceIdiom.Phone ? 40f : 110f;
			CGSize size = new CGSize ((float)tableView.Bounds.Width - margin, float.MaxValue);
	...
	public class EntryElement : Element {
		...
		CGSize ComputeEntryPosition (UITableView tv, UITableViewCell cell)
		{
			...
			s.EntryAlignment = new CGSize ((float)(25 + Math.Min (max.Width, 160)), (float)max.Height);
			return s.EntryAlignment;
		}
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			if (entry == null){
				...
				entry = CreateTextField (new CGRect (size.Width, yOffset, width, size.Height));
	...
	public class DateTimeElement : StringElement {
		...
		{                                                                                                                                    
			...
			case UIInterfaceOrientation.LandscapeRight:
				fX = (float)((screenRect.Height - size.Width) / 2);
				fY = (float)((screenRect.Width - size.Height) / 2 -17);
				break;
				
			case UIInterfaceOrientation.Portrait:
			case UIInterfaceOrientation.PortraitUpsideDown:
				fX = (float)((screenRect.Width - size.Width) / 2);
				fY = (float)((screenRect.Height - size.Height) / 2 - 25);
				break;
			}
